  ...
  return (
    <div className='home'>
        <div className='hero flex justify-center flex-col items-center'>
          <div className='header-txt text-7xl mt-20 mb-4 font-bold'>Crypto Track</div>
          <div className='header-desc text-gray-200 text-xl'><p>Crypto Currency Tracking system</p></div>
          <form onSubmit={searchHandler} className='mt-24 bg-gray-900 rounded-xl border-solid border-gray-400 border-2 outline-none'>
            <input onChange={inputHandler} list='coinlist' value={input} className='search-bar px-4 py-2 h-14 w-140 text-left rounded-xl outline-none ' type='text' placeholder='Search Crypto..'></input>
          ...
          <div className='table-container px-4 py-4 my-10  border-solid border-gray-400 border-2 bg-gray-800 rounded-xl text-xl'>
            <div className='crypto-table px-2 py-3 border-b-solid border-b-gray-400 border-b-2 content-center'>
              <p>#</p>
              <p>Coins</p>
              <p>Price</p>
              <p className='HCh'>24H Change</p>
            ...
            </div>
            {
              displayCoin.slice(0, 10).map((item, index)=>(
                <Link to={`/coin/${item.id}`} key={index} className='crypto-table px-2 py-3  border-b-solid border-b-gray-700 border-b-2'>
                  ...
                  <p className={item.price_change_percentage_24h > 0 ? 'HCh text-green-400 content-center' : 'HCh text-red-500 content-center'}>
                    {Math.floor(item.price_change_percentage_24h * 100)/100}
                  </p>
                  <p className='market-cap text-right content-center'>{currency.symbol} {item.market_cap.toLocaleString()}</p>
                </Link>
              ))
            }
          </div>
